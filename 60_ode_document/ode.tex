...
\subsubsection{How were results interpreted (feature importance, SHAP value, partial dependence, etc.)?} For both models, the baseline CNN and the TerraMind GFM, no XAI method is applied, due to time constraints. For future work, it would be interesting to apply XAI methods to both model architectures, e.g., Grad-CAM for the CNN or attention map analysis for the GFM.
...
\subsubsection{What are the known limitations, failure modes, generalization caveats, and conditions for reliable use?} \label{sec:limit} (Failure analysis on the test of for the 4 best models has shown that certain OilSlick images still do not meet the desired quality to be classified correctly, even after filtering.)
