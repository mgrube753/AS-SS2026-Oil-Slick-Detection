...
\subsubsection{Why is ML an appropriate approach here (what alternative / statistical baselines would miss)?} Oil spills cause severe ecological damage. Furthermore, it is expensive to monitor the oceans manually. ML-based predictions are useful for scalable detection and monitoring in this scenario.
...
\subsubsection{Is an Exploratory Data Analysis (EDA) available? If so, where?} The mentioned EDA notebook is available in the GitHub repository\footnote{\url{https://github.com/mgrube753/AS-SS2026-Oil-Slick-Detection/tree/main/20_data_analysis}, accessed 13/05/2026}.
...
\subsubsection{What steps were taken to check overfitting / underfitting?} To detect overfitting / underfitting, training and validation loss curves \textcolor{red}{will be} logged during training to check for any divergence (overfitting) or convergence (with high loss: underfitting). The mentioned early stopping procedure is implemented to stop training when validation loss does not improve.
