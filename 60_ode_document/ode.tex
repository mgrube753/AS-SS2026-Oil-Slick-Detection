...
\begin{itemize}
    \item Contributed to exploratory data analysis (\texttt{oilslick\_eda.ipynb}) with Vishnu, including value range analysis and band statistics.
    \item Designed the initial project structure and coordinated task management.
    \item Developed \texttt{dataset.py} and contributed to \texttt{dataloader.py} with Suma.
    \item Implemented z-score normalization and percentile clipping in the notebook and data loader.
    \item Implemented filtered metadata and normalization statistics generation, integrated the Baseline CNN and TerraMind models into the training pipeline, and added MLflow experiment logging.
\end{itemize}
\subsection*{Suma}
\begin{itemize}
    \item Documented the \texttt{30\_experiments} module, covering the training pipeline, execution, evaluation, and failure analysis.
    \item Designed and created the scientific poster, iteratively refining the layout, technical content, and formatting to comply with project and academic presentation standards.
    \item Implemented preprocessing and augmentation transforms in \texttt{dataloader.py} for all data splits.
    \item Contributed to the implementation of filtered metadata and normalization statistics.
    \item Involved in discussions on the experimental workflow, documentation, and preprocessing pipeline.
\end{itemize}
\subsection*{Vishnu}
\begin{itemize}
    \item Contributed to exploratory data analysis with Malte.
    \item Implemented $90^\circ$ rotation augmentation in the data loading pipeline.
    ...
    \item Contributed to metadata filtering and normalization statistics.
    \item Implemented early stopping, checkpointing, and MLflow-based model selection.
\end{itemize}
